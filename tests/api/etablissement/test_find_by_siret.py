"""Tests for etablissement find_by_siret module."""
...
@pytest.mark.requirement("REQ-API-015")
class TestGetKwargs:
    """Test _get_kwargs function."""

    def test_get_kwargs_with_siret_and_params(self):
        """Test _get_kwargs with siret and all parameters."""
        kwargs = _get_kwargs(
        ...
        assert kwargs["url"] == "/siret/12345678901234"
    ...
    def test_get_kwargs_with_siret_only(self):
        """Test _get_kwargs with siret only."""
        kwargs = _get_kwargs(siret="12345678901234")
...
@pytest.mark.requirement("REQ-API-016")
class TestParseResponse:
    ...
    def test_parse_response_200_success(self):
        ...
        mock_response.json.return_value = {"etablissement": {}}
        ...
        assert isinstance(result, ReponseEtablissement)
...
@pytest.mark.requirement("REQ-API-017")
class TestBuildResponse:
    ...
    def test_build_response_success(self):
        ...
        with patch(
            "sirene_api_client.api.etablissement.find_by_siret._parse_response"
        ) as mock_parse:
            mock_parse.return_value = ReponseEtablissement.from_dict(
                {"etablissement": {}}
            )
...
@pytest.mark.requirement("REQ-API-018")
class TestSyncDetailed:
    ...
    def test_sync_detailed_with_minimal_params(self):
        ...
        result = sync_detailed(siret="12345678901234", client=mock_client)

        assert result.status_code == HTTPStatus.OK
        mock_httpx_client.request.assert_called_once()


@pytest.mark.requirement("REQ-API-019")
class TestSync:
    """Test sync function."""
    ...
    def test_sync_success(self):
        ...
        with patch(
            "sirene_api_client.api.etablissement.find_by_siret.sync_detailed"
        ) as mock_sync_detailed:
            mock_sync_detailed.return_value.parsed = ReponseEtablissement.from_dict(
                {"etablissement": {}}
            )
...
@pytest.mark.requirement("REQ-API-020")
...
@pytest.mark.requirement("REQ-API-021")
class TestAsyncio:
    ...
    async def test_asyncio_success(self):
        ...
        with patch(
            "sirene_api_client.api.etablissement.find_by_siret.asyncio_detailed"
        ) as mock_asyncio_detailed:
            mock_asyncio_detailed.return_value.parsed = ReponseEtablissement.from_dict(
                {"etablissement": {}}
            )
